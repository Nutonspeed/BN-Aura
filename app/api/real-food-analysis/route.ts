...
import RealFoodAnalysisEngine from '@/lib/ai/realFoodAnalysisEngine';

// API route สำหรับ AI Food Analysis แบบจริง
export async function POST(request: NextRequest) {
  try {
    ...
    // เริ่มการวิเคราะห์
    const analysisEngine = new RealFoodAnalysisEngine();
    ...
    console.error('Real food analysis API error:', error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Get food analysis history error:', error);
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error('Update food preferences error:', error);
    return NextResponse.json(
      { error: 'ไม่สามารถอัปเดตค่ากำหนดได้' },
      { status: 500 }
    );
  }
}
