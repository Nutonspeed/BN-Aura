      ...
      <div className="relative">
        ...
        <div className="space-y-4">
          {stages.map((stage, index) => {
            ...
            const isปัจจุบัน = stage === currentStage;
            const isรอดำเนินการ = index > currentIndex;
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isปัจจุบัน && 'bg-primary/20 border-primary animate-pulse',
                    isรอดำเนินการ && 'bg-background border-border'
                  )}
                >
                  {isCompleted ? (
                ...
                <div className="flex-1">
                  <div
                    className={cn(
                      'text-sm font-medium',
                      isปัจจุบัน && 'text-primary',
                      isCompleted && 'text-foreground',
                      isรอดำเนินการ && 'text-muted-foreground'
                    )}
                  >
                    {stageInfo.icon} {stageInfo.label}
                  </div>
                  {isปัจจุบัน && (
                    <div className="text-xs text-muted-foreground">ปัจจุบัน stage</div>
                  )}
                </div>
              </motion.div>
            );
