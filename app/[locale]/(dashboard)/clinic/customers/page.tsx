                ...
                >
                  <td className="px-8 py-6 relative">
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div className="flex flex-col space-y-1 pr-6">
                        <span className="text-base font-black text-white group-hover:text-primary transition-colors tracking-tight">{customer.name}</span>
                        <div className="flex flex-wrap items-center gap-3 text-[10px] text-muted-foreground font-medium italic">
                          <span className="flex items-center gap-1.5"><Mail className="w-3 h-3 text-primary/60" /> {customer.email}</span>
                          <div className="w-1 h-1 rounded-full bg-white/10" />
                          <span className="flex items-center gap-1.5"><Phone className="w-3 h-3 text-primary/60" /> {customer.phone}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="space-y-2 w-48">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xs font-black text-white">{customer.skinScore}%</span>
                      </div>
                      <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden p-0.5 border border-white/5">
                        <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${customer.skinScore}%` }}
                          transition={{ duration: 1.5, ease: "easeOut", delay: 1 }}
                          className={cn(
                            "h-full rounded-full shadow-sm",
                            customer.skinScore > 80 ? "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]" : 
                            customer.skinScore > 70 ? "bg-primary shadow-[0_0_10px_rgba(var(--primary),0.4)]" : 
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col space-y-1.5">
                      ...
                      <span className="text-xs text-white/60 font-medium tabular-nums">{new Date(customer.lastScan).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                    </div>
                  </td>
